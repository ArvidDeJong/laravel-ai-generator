...
test('Blade examples are wrapped in raw, so Liquid does not eat them', function () {
    foreach (glob(docsPath('*.md')) as $page) {
        ...
        // toContain() takes several needles, so a message as second argument would turn the negated
...
test('the config holds the package facts and the sitemap plugin', function () {
    expect(file_get_contents(docsPath('_config.yml')))
        ...
        ->toContain('name: darvis/laravel-ai-generator')
        ->toContain('baseurl: /laravel-ai-generator')
...
test('every link between pages resolves and every page is linked from the home page', function () {
    ...
    foreach (glob(docsPath('*.md')) as $page) {
        ...
        if ($name !== 'index.md') {
            expect(str_contains($index, ']('.$name))->toBeTrue($name.' is not linked from index.md');
        }
...
test('the docs warn that an image is generated by default', function () {
    // includeImage defaults to true and costs a second, billed call: a beginner has to read that
    // before the first request, on the pages where the first request is shown.
    foreach (['index.md', 'installation.md', 'usage.md'] as $page) {
        expect(str_contains((string) file_get_contents(docsPath($page)), 'includeImage: false'))
            ->toBeTrue($page.' shows a first request without includeImage: false');
    }
});
